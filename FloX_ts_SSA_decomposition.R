...
#3. In case of time series of fluorescence retrieved with spectral fitting method (SFM),
#is needed to filter out fluorescence values when optimization of spectral fitting
...
ts_chosen_var<-subset(ts_filt_aggr_ext, select=c("no_data","datetime","Lat","Lon","doy.dayfract","SZA","SAA","PAR..W.m.2.",
                                                 ...
                                                 "SIF_B_sfm..mW.m.2nm.1sr.1.", "PRI", "NDVI", "MTCI"))
...
ts_chosen_var$SAA<-sunAngle(as.POSIXct(ts_chosen_var$datetime, tz="GMT"), lat=unique(ts_chosen_var$Lat)[!is.na(unique(ts_chosen_var$Lat))], 
                            lon=unique(ts_chosen_var$Lon)[!is.na(unique(ts_chosen_var$Lon))],
...
ts_chosen_var$Lat<-unique(ts_chosen_var$Lat)[!is.na(unique(ts_chosen_var$Lat))] 
ts_chosen_var$Lon<-unique(ts_chosen_var$Lon)[!is.na(unique(ts_chosen_var$Lon))]
